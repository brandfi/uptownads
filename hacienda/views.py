...
def index(request):
    ...
    url = 'http://' + request.get_host() + \
        reverse('hacienda:check-credentials')
    ...
    return render(request, 'hacienda/index.html', context)
...
def check_credentials(request):
    ...
    if r.status_code == 200:
        ...
        successs_url = 'http://' + request.get_host() + \
            reverse('hacienda:success')
        context = {"username": user['username'],
                        "password": user['value'],
                        "l_url": login_url,
                        "dst": successs_url,
                        "popup": True}
        return render(request, 'hacienda/submit.html', context)
        # r = requests.post(login_url, params=login_params)
        # return HttpResponseRedirect(successs_url)
    else:
        return HttpResponseRedirect(reverse('hacienda:signup'))
...
def signup(request):
    if request.method == 'POST':
        ...
        sms_r = requests.post(
            ...
            json=sms_params,
            headers=headers)
        return HttpResponseRedirect(reverse('hacienda:verify'))

    terms_url = 'http://' + request.get_host() + \
        reverse('hacienda:terms')

    context = {
        'terms_url': terms_url,
    }
    return render(request, 'hacienda/signup.html', context)
...
def verify(request):
    ...
    return render(request, 'hacienda/verify.html', context)


def success(request):
    return render(request, 'hacienda/success.html')


def terms(request):
    return render(request, 'hacienda/terms.html')
