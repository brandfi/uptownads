...
def index(request):
    ...
    return render(request, 'brewbistro/index.html', context)
...
def check_credentials(request):
    ...
    if r.status_code == 200:
        ...
        return HttpResponseRedirect(reverse('brewbistro:signup'))
...
def signup(request):
    if request.method == 'POST':
        ...
        sms_r = requests.post(
            ...
            json=sms_params,
            headers=headers)
        return HttpResponseRedirect(reverse('brewbistro:verify'))

    terms_url = 'http://' + request.get_host() + \
        reverse('brewbistro:terms')

    context = {
        'terms_url': terms_url,
    }
    return render(request, 'brewbistro/signup.html', context)
...
def verify(request):
    ...
    return render(request, 'brewbistro/verify.html', context)


def success(request):
    return render(request, 'brewbistro/success.html')


def terms(request):
    return render(request, 'brewbistro/terms.html')
